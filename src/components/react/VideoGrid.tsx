...
export default function VideoGrid({ videos }: VideoGridProps) {
  ...
  if (videos.length === 0) {
    return <div className="end-message">{t('videos.noVideos')}</div>;
  }
    ...
    <>
                ...
                <div className="video-meta">
                  ...
                  {video.viewCount != null && (
                    <span className="video-views">
                      {t('videos.views', { count: video.viewCount.toLocaleString(dateLocale) })}
                    </span>
                  )}
                </div>
              </div>
      ...
      >
        <div className="video-modal-inner">
          <button className="video-modal-close" onClick={closeModal} aria-label={t('videos.close')}>✕</button>
          {activeVideo && (
            <>
